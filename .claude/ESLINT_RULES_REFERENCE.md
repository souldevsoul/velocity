...
This document lists all custom ESLint rules used in Velocity to enforce product quality, brand consistency, and prevent common mistakes.
...
- Template colors that shouldn't be in Velocity
...
'product-quality/consistent-company-info': ['error', {
  ...
  email: 'support@velocity.dev',
}]
```

**Detects:**
- Wrong company name: "VoiceCraft", "velocity", "LOGOSMITH"
- Wrong email: "info@...", "hello@...", etc.

**Example Violation:**
```tsx
<a href="mailto:info@velocity.com">  // ❌ Wrong email
  Contact Us
</a>
```
...
**Correct Usage:**
```tsx
<a href="mailto:support@velocity.dev">  // ✅ Correct email
  Contact Us
</a>
```
...
**Correct Usage:**
```tsx
<p>Choose from 7 Logo Design Styles</p>  // ✅ Velocity content
```
...
**Correct Usage:**
```tsx
<img src="/logo.png" alt="Velocity logo" />  // ✅ Has alt text

<Image
  src="/logo.png"
  alt="Velocity logo"  // ✅ Has alt text
  width={200}
  height={200}
/>
```
...
export default function Terms() {
  return (
    <div>
      ...
      <p>By accessing and using Velocity...</p>
      {/* ... real terms content ... */}
    </div>
  )
}
...
Copy `eslint.config.product.mjs` from Velocity to your project.
...
**Tested On:** Velocity (Next.js 16, TypeScript)
