        ...
        <div className="container mx-auto px-4">
          <div className="relative flex h-20 items-center">
            ...
            <Link className="inline-flex items-center gap-2.5 group" href={getRouteByName('home')} aria-label="Velocity - Home">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-600 to-green-700 rounded-lg flex items-center justify-center transition-transform group-hover:scale-105">
              ...
              <span className="text-2xl font-semibold text-gray-900">Velocity</span>
            </Link>

            {/* Mobile menu button */}
            <button
              className="lg:hidden flex items-center justify-center h-10 w-10 ml-auto bg-gradient-to-b from-emerald-600 to-green-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2"
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 hidden lg:flex items-center">
              {navigationItems.map((item, index) => (
                <Link
                  key={item.to}
                  className={`inline-block text-gray-900 hover:text-emerald-600 ${index < navigationItems.length - 1 ? 'mr-10' : ''} font-medium transition duration-200 focus:outline-none focus:underline ${isActiveRoute(item.to) ? 'text-emerald-600' : ''}`}
                  href={item.to}
            ...
            <div className="hidden lg:block ml-auto">
              <Link
                className="inline-flex items-center justify-center h-10 mr-4 px-4 text-center leading-loose text-sm text-gray-900 hover:text-emerald-600 font-semibold transition duration-200 focus:outline-none focus:underline"
                href={getRouteByName('login')}
              >
                Login
              </Link>
              <Link
                className="inline-flex items-center justify-center h-10 px-4 text-center leading-loose text-sm text-white font-semibold border border-emerald-600 bg-emerald-600 hover:bg-emerald-700 shadow-sm rounded-lg transition duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
        ...
        >
          ...
          <nav className="relative flex flex-col py-6 px-10 w-full h-full bg-white overflow-y-auto" role="navigation">
            ...
            <div className="py-12 mb-auto">
              <ul className="flex-col space-y-6" role="list">
                {navigationItems.map((item) => (
                  <li key={item.to}>
                    <Link
                      className={`inline-block text-lg font-medium transition duration-200 focus:outline-none focus:underline ${isActiveRoute(item.to) ? 'text-emerald-700' : 'text-black hover:text-emerald-700'}`}
                      href={item.to}
                      onClick={closeMobileMenu}
                      aria-current={isActiveRoute(item.to) ? 'page' : undefined}
                    >
            ...
            <div className="space-y-4">
              ...
              <Link
                className="flex items-center justify-center h-10 px-4 text-center text-sm text-white font-semibold border border-emerald-600 bg-emerald-500 hover:bg-emerald-600 rounded-lg transition duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
                href={getRouteByName('register')}
                onClick={closeMobileMenu}
              >
                Get Started
